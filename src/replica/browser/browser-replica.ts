...
export interface BrowserReplicaOptions {
   ...
   * `new Worker(new URL(...))`. The worker must run this package's db.worker module.
   */
  createWorker?: () => Worker;
...
export class BrowserReplica {
  ...
  constructor(handlers: ReplicaHandlers, options: BrowserReplicaOptions) {
    ...
    this.deltas = new DeltaQueue({
      ...
      // Backlog deeper than it is worth replaying — take the snapshot instead. The queue has already
      // dropped what it held; reseed() replaces the whole DB and sets the cursor to the snapshot head,
      // so dropping those frames leaves no hole.
      onOverflow: (depth) => {
        console.warn(
  ...
  close(): void {
    ...
    for (const slot of this.pending.values()) {
      slot.reject(new Error("replica client closed"));
    }
    this.pending.clear();
